...
func spawn_hero() -> void:
	...
	_player_sprite.add_child(_create_blob_shadow())
...
func spawn_mobs(count: int, zone_id: int) -> void:
	...
	for i in range(count):
		...
		sprite.add_child(_create_blob_shadow())
...
func play_attack(target_mob_id: int) -> void:
	...
	tween.tween_property(player_anchor, "position", base_pos, 0.2).set_ease(Tween.EASE_IN)
...
func spawn_damage_number(world_pos: Vector2, amount: int, is_heal: bool = false) -> void:
	var label := Label.new()
	...
	if mat != null:
		...
		get_tree().create_timer(duration).timeout.connect(func():
			if is_instance_valid(sprite):
				...
				if m != null:
					m.set_shader_parameter("flash_active", false)
		)
	else:
...
func _create_blob_shadow() -> Sprite2D:
