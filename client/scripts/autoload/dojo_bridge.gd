...
const ABILITY_SLOT_STATE_MODEL := "athanor_v2-AbilitySlotState"
const TELEGRAPH_STATE_MODEL := "athanor_v2-TelegraphState"
const ACTIONS_CONTRACT_TAG := "athanor_v2-actions_v2"
...
func spawn_v2(class_id: int) -> void:
	_execute_action("spawn_v2", [class_id])
...
func _create_entity_subscription() -> void:
	...
	if ClassDB.class_exists("KeysClause"):
		...
		clause.call("add_model", ABILITY_SLOT_STATE_MODEL)
		clause.call("add_model", TELEGRAPH_STATE_MODEL)
		clause.call("pattern", 2)  # VariableLen
		push_warning("[dojo_bridge] Subscribing with KeysClause: %s, %s, %s, %s, %s" % [RUN_STATE_MODEL, ROOM_STATE_MODEL, ACTOR_STATE_MODEL, ABILITY_SLOT_STATE_MODEL, TELEGRAPH_STATE_MODEL])
	else:
...
func _handle_entity_payload(payload: Dictionary) -> void:
	...
	if models.has(RUN_STATE_MODEL):
		var run_model := _normalize_model(models[RUN_STATE_MODEL])
		if _matches_current_player(run_model):
			var run_gid := int(run_model.get("game_id", -1))
			if run_gid > target_gid:
				game_state.set_latest_game_id(run_gid)
				target_gid = run_gid
			if run_gid == target_gid and run_gid > 0:
				game_state.update_run(run_model)
			elif run_gid > 0:
				_store_historical_run(run_model)

	if models.has(ROOM_STATE_MODEL):
		var room_model := _normalize_model(models[ROOM_STATE_MODEL])
		if _matches_current_player(room_model):
			var room_gid := int(room_model.get("game_id", -1))
			if room_gid == target_gid and room_gid > 0:
				game_state.update_room(room_model)

	if models.has(ACTOR_STATE_MODEL):
		var actor_model := _normalize_model(models[ACTOR_STATE_MODEL])
		if _matches_current_player(actor_model):
			var actor_gid := int(actor_model.get("game_id", -1))
			if actor_gid == target_gid and actor_gid > 0:
				game_state.upsert_actor(actor_model)

	if models.has(ABILITY_SLOT_STATE_MODEL):
		var ability_model := _normalize_model(models[ABILITY_SLOT_STATE_MODEL])
		if _matches_current_player(ability_model):
			var ability_gid := int(ability_model.get("game_id", -1))
			if ability_gid == target_gid and ability_gid > 0:
				game_state.upsert_ability_slot(ability_model)

	if models.has(TELEGRAPH_STATE_MODEL):
		var telegraph_model := _normalize_model(models[TELEGRAPH_STATE_MODEL])
		if _matches_current_player(telegraph_model):
			var telegraph_gid := int(telegraph_model.get("game_id", -1))
			if telegraph_gid == target_gid and telegraph_gid > 0:
				game_state.upsert_telegraph(telegraph_model)

var _history_runs := {}  # game_id → RunState dict

func _store_historical_run(model: Dictionary) -> void:
	var gid := int(model.get("game_id", -1))
	if gid < 0:
		return
	_history_runs[gid] = model
	var phase := int(model.get("phase", 0))
